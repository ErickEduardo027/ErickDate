    ...
    <div>
      ...
      <div className="grid gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        {data.tv_shows.map((show: any) => (
          <Link
            key={show.id}
            to={`/show/${encodeURIComponent(show.permalink)}`} // 👈 ruta hacia detalles
            className="bg-neutral-900 border border-neutral-800 rounded-xl overflow-hidden hover:scale-105 transition-transform block"
          >
            <img
